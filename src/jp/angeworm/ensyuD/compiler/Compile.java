...
class CompileImpl {
	...
	public Environment makeEnvironment(Procedure pl, Environment env) {
		...
		for(Variable arg : pl.args) {
			...
			int arrayLabel = 0;
			if(arg.type instanceof ArrayType) arrayLabel = ((ArrayType)(arg.type)).min;
			e.addVariable(arg, Location.LocationType.StackLocation, stackSize - stackSum - arrayLabel);
		}
		...
		for(Variable var : pl.vars) {
			...
			int arrayLabel = 0;
			if(var.type instanceof ArrayType) arrayLabel = ((ArrayType)(var.type)).min;
			e.addVariable(var, Location.LocationType.StackLocation, varsSize - stackSum - arrayLabel);
		}
		...
		for(Environment.EnvironmentEntry ent : env) {
			...
			if(ent.val.type instanceof ArrayType) {
				...
				for(; min <= t.max; min++) {
					...
					code.append(lb + " DC " + getValue(ent.defaultValue, min - i) + cmt + "\n");
				}
			} else if (ent.val.type instanceof FunctionType) {
				continue;
			} else {
				code.append(ent.loc.getLabel() + " DC "+ getValue(ent.defaultValue, 0) + "; " + ent.val.name + "\n");
			}
		}
	}
	...
	public void parse(StringBuilder code, Procedure pl, Environment e, StringBuilder outercode){
		...
		code.append(prgname + " LAD  GR8, " + -getSize(pl.vars) + ", GR8 ; " + pl.name + "\n");
		
		code.append(spc + " ; Environment:" + "\n");
		for(EnvironmentEntry v : env) {
			code.append(spc + " ; " + v.val.name + ":" + v.val.type + ":" + " GR8,"+ v.loc.getId() + "\n");
		}
		...
		if (s instanceof AssignSentence) {
			...
			if(ent.loc.type.equals(LocationType.HeapLocation)) {
				if(lvalue.index != null) {
					...
					code.append(spc + " ST   GR" + rvalue + ", " + ent.loc.getLabel() + ", GR" + indexReg + " ; " + lvalue.value + "[] := " + "\n");
					freeRegister(code);
				} else {
					code.append(spc + " ST   GR" + rvalue + ", " + ent.loc.getLabel() + " ; " + lvalue.value + " := " + "\n");
				}
			} else if(ent.loc.type.equals(LocationType.StackLocation)) {
				if(lvalue.index != null) {
					int indexReg = parseValue(code, lvalue.index, e);
					code.append(spc + " LAD  GR" + indexReg + ", " + (ent.loc.getId() + rs.getStackHeight()) + ", GR" + indexReg + "\n");
					code.append(spc + " ADDL GR" + indexReg + ", GR8" + "\n");
					code.append(spc + " ST   GR" + rvalue + ", 0, " + ", GR" + indexReg + " ; " + lvalue.value + "[] := " + "\n");
					freeRegister(code);
				} else {
					int indexReg = takeRegister(code);
					code.append(spc + " LAD  GR" + indexReg + ", " + (ent.loc.getId() + rs.getStackHeight()) + "\n");
					code.append(spc + " ADDL GR" + indexReg + ", GR8" + "\n");
					code.append(spc + " ST   GR" + rvalue + ", 0, GR" + indexReg + " ; " + lvalue.value + " := " + "\n");
					freeRegister(code);
				}
			} else {
				throw new RuntimeException("unknown Location Type" + ent.loc.type);
			}
			freeRegister(code);
			...
			if(ent.loc.type.equals(Location.LocationType.HeapLocation)) {
				if(ent.val.type instanceof ArrayType) {
					...
					if(vas.index != null) {
						...
						code.append(spc + " LD   GR" + valReg + ", " + ent.loc.getLabel() + ", GR" + indexReg + " ; " + s.value + "\n");
						...
						code.append(spc + " LAD  GR" + valReg + ", " + ent.loc.getLabel() + " ; " + s.value + "\n");
						code.append(spc + " LAD  GR" + valReg + ", " + ((ArrayType) ent.val.type).min + ", GR" + valReg + " ; " + s.value + "\n");
					...
					code.append(spc + " LD   GR" + valReg + ", " + ent.loc.getLabel() + " ; " + s.value + "\n");
				...
				if(ent.val.type instanceof ArrayType) {
					...
					code.append(spc + " LAD  GR" + valReg + ", " + (ent.loc.getId() + rs.getStackHeight()) + ", GR8" + " ; " + s.value + "\n");
					...
					code.append(spc + " LAD  GR" + valReg + ", " + (ent.loc.getId() + rs.getStackHeight()) + ", GR8" + " ; " + s.value + "\n");
					code.append(spc + " LD   GR" + valReg + ", 0, GR" + valReg + " ; " + s.value + "\n");
